...
void
comma (vm_t *vm, cell_t x)
{
  *(cell_t *)allot (vm, sizeof (cell_t)) = x;
}

xt_t
entry_xt (dict_entry_t *w)
{
  uintptr_t name_end
      = (uintptr_t)w + offsetof (dict_entry_t, name) + w->name_len;
...
static int
name_eq (const char *a, const char *b, size_t len)
{
...
dict_entry_t *
find_word (vm_t *vm, const char *name, size_t len)
{
